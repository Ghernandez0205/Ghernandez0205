...
def generar_oficio(data, num_oficio, sede, ubicacion, fecha_comision, horario, mes_emision, comision):
    ...
    for _, fila in data.iterrows():
        ...
        for p in doc.paragraphs:
            ...
            p.text = p.text.replace("fecha", fecha_comision.strftime('%d de %B del %Y').replace('January', 'enero').replace('February', 'febrero').replace('March', 'marzo').replace('April', 'abril').replace('May', 'mayo').replace('June', 'junio').replace('July', 'julio').replace('August', 'agosto').replace('September', 'septiembre').replace('October', 'octubre').replace('November', 'noviembre').replace('December', 'diciembre'))
